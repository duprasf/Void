...
class Dice {
    ...
    */
    static public function max($param, $reroll1 = false, $removeLowest = false)
    {
        ...
        $param = preg_replace_callback('((\d*)d(\d+))i', function($matches) use ($reroll1, $removeLowest){
            if(!is_numeric($matches[1])) $matches[1] = 1;
            // since all dice will have the same value, if you're supposed to remove the lowest die, just remove one.
            //if($removeLowest) $matches[1]--;
            // since the $removeLowest now ADDS a dice by itself, we just ignore this param
    ...
    */
    static public function min($param, $reroll1 = false, $removeLowest = false)
    {
        ...
        $param = preg_replace_callback('((\d*)d(\d+))i', function($matches) use ($reroll1, $removeLowest){
            if(!is_numeric($matches[1])) $matches[1] = 1;
            // since all dice will have the same value, if you supposed to remove the lowest die, just remove one.
            if($removeLowest) $matches[1]--;
